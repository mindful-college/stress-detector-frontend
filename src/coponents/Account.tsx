    ...
    const account = async () => {
      try {
        ...
        if (res.status === 200) {
          ...
          dispatch({ type: 'SET_USER', payload: updatedUserInfo });
        }
      } catch (error) {
    ...
    <View>
      ...
      <View style={styles.block}>
        {isEnabled ? (
          ...
          <Text style={styles.item}>{state.user?.name}</Text>
        )}
        <TouchableOpacity
          style={styles.button}
      ...
      <View style={styles.block}>
        <Text style={styles.item}>{state.user?.points} Points</Text>
      </View>
    </View>
  );
}
